...
from cherrant.constant import DATA_PATH
...
def read_cilin():
    ...
    lines = open(os.path.join(DATA_PATH, "knowledge", "cilin.txt"), "r", encoding="gbk").read().strip().split("\n") # yuhuang add
    semantic_dict = {}
    semantic_classes = {}
    for line in lines:
...
def read_confusion():
    ...
    with open(os.path.join(DATA_PATH, "knowledge", "confusion_dict.txt"), "r", encoding="utf-8") as f: # yuhuang add
        for line in f:
            li = line.rstrip('\n').split(" ")
